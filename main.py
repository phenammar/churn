...
async def home():
    return {"massage": f"Welcome to {APP_NAME} API v{VERSION}"}
...
async def predict_forest (data: CustomerData, api_key: str=Depends(verify_api_key)) -> dict:
    try:
            result=predict_churn(data=data, preprocessor=preprocessor, model=forest_model)
            return result
...
async def predict_xgboost(data: CustomerData, api_key: str=Depends(verify_api_key)) -> dict:
   ...
   try:
       result = predict_churn(data=data, preprocessor=preprocessor, model=xgboost_model)
       return result
